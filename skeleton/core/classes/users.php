...
class Users{
	...
	public function insert_user($username, $password, $usertype, $date_created)
	{
		global $db;

		$query = $this->db->prepare("	INSERT INTO `users` (`username`, `password`,`usertype`, `date_created`) 
										VALUES 		(?, ?, ?, ?)");
		
		$query->bindValue(1, $username);
		$query->bindValue(2, $password);	
		$query->bindValue(3, $usertype);	
		$query->bindValue(4, $date_created);

		try{
			$query->execute();
			$merchant_user_id = $db->lastInsertId(); 
			header('Location:merchant_add.php?merchant_user_id='.$merchant_user_id);
	...
	public function get_users() {
		...
		$query = $this->db->prepare("SELECT * FROM `users` ORDER BY `user_id` DESC");
